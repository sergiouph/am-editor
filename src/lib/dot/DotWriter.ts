...
export class DotWriter {
    ...
    public writeProperty(name: string, value: string | boolean | null | undefined) {
        this.writeToken(name)
        this.writeToken('=')
        if (value === null || value === undefined) {
            this.writeToken('null')
        } else if (typeof value === 'boolean') {
            this.writeToken(value ? 'true' : 'false')
        } else {
            this.writeString(value)
        }
    }

    public writeProperties(properties: Record<string, string | null>) {
